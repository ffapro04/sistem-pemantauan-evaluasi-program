  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:3000/assessment?jenis=non-akademik&id_ho=${id_ho}`,
        {
          headers: {
  ...
  return (
    <PageWrapper className="h-screen bg-white flex overflow-hidden !p-0 font-sans selection:bg-[#0AC4E0]/20 text-slate-800">
          ...
          <header className="flex flex-row items-center justify-between animate-in fade-in duration-1000 leading-none">
            <div className="space-y-3">
              ...
              <h1 className="text-3xl font-black text-slate-800 tracking-tighter">
                Riwayat Assessment <span className="text-[#0AC4E0]">Non-Akademik</span>
              </h1>
            </div>
            ...
            <Button
              ...
              className="!bg-slate-800 hover:!bg-[#0AC4E0] !text-white !rounded-2xl !px-8 !py-4 !text-sm !font-bold shadow-xl transition-all duration-300 active:scale-95 leading-none border-none"
            />
          </header>
              ...
              <Table
                columns={[
                    ...
                    render: (row) => (
                      <div className="flex flex-col gap-1 py-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Non-Akademik</span>
                        </div>
                      </div>
                    ),
                  },
                  ...
                  },
                  {
                    ...
                    render: (row) => {
                      const schools = row.sekolah && row.sekolah !== "-" ? row.sekolah.split(",") : [];
                      return (
                        <div className="flex flex-wrap gap-1.5 max-w-[220px] py-1">
                          {schools.length > 0 ? (
                            schools.map((nama, idx) => (
                  ...
                  },
                  {
                    ...
                    render: (row) => (
                      <div className="flex justify-end items-center gap-2">
                        <button
                          onClick={() => navigate(`/ho/assessment/non-akademik/detail/${row.id}`)}
                        ...
                        {!row.sent && (
                          <button
                            onClick={() => navigate(`/ho/assessment/non-akademik/edit/${row.id}`)}
      ...
      <style dangerouslySetInnerHTML={{
        ...
        tbody tr:hover td { background-color: rgba(10, 196, 224, 0.05) !important; transition: all 0.2s ease; }
      `}} />
    </PageWrapper>
  );
}
