...
function ReadProgramnonAkademik() {
  ...
  const [programs, setPrograms] = useState([]); // State untuk menghitung statistik program
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch sekolah dan program non-akademik secara paralel
  ...
  // Filter & Mapping Logic (Agar data statistik Aktif/Draft muncul di Card)
  ...
  return (
    <PageWrapper className="h-screen bg-[#F8FAFC] flex overflow-hidden !p-0 font-sans text-slate-900">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        {/* HEADER AREA - MENIRU ReadProgramAkademik */}
          ...
          <div className="flex items-center justify-between mb-10">
            <div>
              ...
              <h1 className="text-4xl font-black text-slate-900 tracking-tight">
                Program <span className="text-[#0AC4E0]">Non-Akademik</span>
              </h1>
            </div>
              ...
              <p className="mt-4 font-black uppercase tracking-widest text-slate-400">Data Tidak Ditemukan</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                  {currentData.map((item) => (
                    <Card
                      key={item.id_sekolah}
                      className="group !bg-white !rounded-[2.5rem] !p-10 border-2 border-slate-50 shadow-[0_2px_15px_rgba(0,0,0,0.02)] hover:shadow-[0_40px_80px_rgba(0,0,0,0.06)] hover:border-[#0AC4E0]/30 transition-all duration-700 flex flex-col h-[420px] relative overflow-hidden"
                    >
                  ...
                  <div className="pt-8 border-t border-slate-100 z-10 mt-6">
                    <button
                      onClick={() => navigate(`/ho/program/non-akademik/list/${item.id_sekolah}`)}
                      className="w-full flex items-center justify-between group/btn"
                    >
                      <span className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-900 group-hover/btn:text-[#0AC4E0] transition-colors">Daftar Program Non-Akademik</span>
