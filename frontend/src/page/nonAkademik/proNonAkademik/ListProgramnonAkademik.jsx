...
function ListProgramnonAkademik() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [resProgram, resSekolah, resUsers] = await Promise.all([
        fetch("http://localhost:3000/program?kategori=NON_AKADEMIK", { headers }),
  ...
  const handleToggleStatus = async (programId, currentStatus) => {
    try {
      ...
      if (res.ok) {
        toast.success(`Status program diperbarui ke ${newStatus}`);
        fetchData();
      }
    } catch (err) {
  ...
  const tableColumns = [
    {
      header: "NO",
      align: "text-center w-[70px]", // Header & Value di tengah
      render: (_, index) => (
    ...
    {
      header: "PROGRAM NON-AKADEMIK",
      align: "text-left", // Value rata kiri
      render: (row) => (
    ...
    },
    {
      ...
      align: "text-left", // Value rata kiri
      render: (row) => {
        const pic = masterUsers.find((u) => String(u.id_user || u.id) === String(row.id_ho));
        return (
    ...
    {
      header: "TAHUN",
      align: "text-center", // Header & Value di tengah
      render: (row) => (
    ...
    {
      header: "AKSI",
      align: "text-right w-[160px]", // Value rata kanan
      render: (row) => (
    ...
    <PageWrapper className="h-screen bg-[#FBFBFD] flex overflow-hidden !p-0 font-sans selection:bg-[#0AC4E0]/20 text-slate-800">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative leading-none">
        ...
        <div className="flex-1 flex flex-col px-6 md:px-10 pt-10 pb-4 overflow-hidden leading-none">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4">
                <button
                  ...
                  className="mt-1 w-10 h-10 flex items-center justify-center bg-white border border-slate-100 rounded-full hover:bg-[#0AC4E0] hover:text-white transition-all shadow-sm group active:scale-90 shrink-0"
                >
                  <ArrowLeft size={18} />
                </button>
                <div className="space-y-3">
                  ...
                  <h1 className="text-3xl font-black text-slate-800 tracking-tighter leading-tight">
                    {sekolahDetail?.nama_sekolah || "Detail Sekolah"} <span className="text-[#0AC4E0]">Non-Akademik</span>
                  </h1>
                </div>
              </div>
            </div>
          ...
          <footer className="mt-6 flex justify-between items-center py-6 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm px-10 leading-none">
            <div className="flex items-center gap-3 text-[11px] font-black text-slate-400 uppercase tracking-widest">
              ...
              {totalItems > 0 ? `Viewing ${currentData.length} of ${totalItems} Programs` : "No programs found"}
            </div>
            <Pagination
              currentPage={page}
