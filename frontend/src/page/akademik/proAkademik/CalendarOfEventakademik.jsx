      ...
      return [
        {
          ...
          title: program.nama_program || "Program Akademik",
...
function CalendarOfEventakademik() {
  ...
  const fetchPrograms = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:3000/program?kategori=AKADEMIK", {
        headers: {
      ...
      if (!res.ok) {
        throw new Error(data?.message || "Gagal mengambil program akademik");
      }

      setPrograms(normalizeArray(data));
    } catch (error) {
      console.error("Gagal mengambil calendar akademik:", error);
      toast.error(error.message || "Gagal mengambil calendar event akademik");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SchedulingCalendarBase
      ...
      subtitle="Lihat jadwal fase dan kegiatan program akademik berdasarkan tanggal pelaksanaan."
      loading={loading}
      schedules={schedules}
      onScheduleClick={(schedule) =>
        navigate(`/ho/program/akademik/detail/${schedule.programId}`)
      }
    />
  );
}
