...
const EditPengurus = () => {
  ...
  // --- OPSI JABATAN (Variabel yang tadi hilang) ---
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.jabatan === "Admin" && adminKey !== MASTER_AUTH_KEY) {
      setStatusNote({ show: true, type: 'error', message: "Master Key Salah!" });
      return;
    }
    ...
    try {
      ...
      setStatusNote({ show: true, type: 'success', message: "Update Berhasil." });
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative items-center justify-end" onMouseUp={handleEnd}>
          ...
          {!isUnlocked && (
            ...
            >
              <div className="text-center mb-16 leading-none">
                ...
                <h2 className="text-3xl font-black tracking-tighter uppercase mb-2 leading-none">Security Access</h2>
                <p className="text-sm text-slate-400 leading-none">Tarik pola otoritas untuk mengedit database</p>
              </div>
        ...
        <motion.div className="mb-8 text-center z-10 leading-none">
          <div className="flex items-center justify-center gap-2 mb-2 leading-none">
            <span className="text-[11px] font-black uppercase tracking-[0.4em] text-[#1C0770]/50 uppercase leading-none">Security Database Protocol</span>
          </div>
          <h1 className="text-5xl font-black tracking-tighter text-[#0AC4E0] uppercase leading-none">Edit Pengurus</h1>
        </motion.div>

        <motion.div
          className="w-full max-w-3xl bg-white rounded-t-[5rem] rounded-b-none shadow-[0_-20px_100px_rgba(10,196,224,0.1)] p-16 pb-48 border-t border-x border-[#0AC4E0]/10 relative z-10 overflow-hidden"
        >
          ...
          <form onSubmit={handleSubmit} className="relative z-10 space-y-10 leading-none">
            <div className="space-y-3 opacity-60 leading-none">
              <Label text="Nama Lengkap (Sistem Terkunci)" className="!text-[10px] !font-black !text-slate-300 !uppercase !tracking-widest !ml-2" />
              <div className="flex items-center gap-6 px-14 py-5 bg-slate-50/50 border border-slate-100 rounded-[2.2rem] relative cursor-not-allowed leading-none">
                <LockIcon className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-200" size={20} />
                <span className="text-[16px] font-bold text-slate-300 uppercase tracking-tight leading-none">{formData.nama}</span>
              </div>
            </div>
            ...
            <div className="space-y-3 leading-none">
              <Label text="Security Code (Password)" required className="!text-[10px] !font-black !text-slate-400 !uppercase !tracking-widest !ml-2" />
              <div className="relative group leading-none">
        ...
        >
          <div className="flex items-center justify-between w-full mb-2">
            <button onClick={() => navigate("/admin/pengurus")} className="flex items-center gap-2 px-8 py-4 bg-white border border-slate-100 text-slate-400 hover:text-slate-800 rounded-full text-[10px] font-black uppercase tracking-widest transition-all active:scale-90 shadow-sm leading-none">
